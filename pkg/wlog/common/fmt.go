// refactor from github.com/x-cray/logrus-prefixed-formatter
...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	if !f.DisableUppercase {
		levelText = strings.ToUpper(f.LogPrefix + levelText)
	}
	...
	if f.DisableTimestamp {
		...
		_, _ = fmt.Fprintf(b, "%s %s%s "+messageFormat, colorScheme.TimestampColor(timestamp), level, prefix, message)
	}
	...
	if entry.HasCaller() {
		if f.FuncName {
			_, _ = fmt.Fprintf(b, " %s=%+v", levelColor("Func"), entry.Caller.Function)
		}
		if f.FileName {
			_, _ = fmt.Fprintf(b, " %s=%+v", levelColor("File"), filepath.Base(entry.Caller.File)+":"+strconv.Itoa(entry.Caller.Line))
		}
	}
